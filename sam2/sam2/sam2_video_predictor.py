...
class SAM2VideoPredictor(SAM2Base):
	...
	# MOD: Add new object to inference state
    @torch.inference_mode()
    def propagate_object_in_video(self, inference_state, obj_id):
        obj_idx = self._obj_id_to_idx(inference_state, obj_id)
        obj_output_dict = inference_state["output_dict_per_obj"][obj_idx]
        obj_temp_output_dict = inference_state["temp_output_dict_per_obj"][obj_idx]
        for is_cond in [False, True]:
    ...
    def propagate_in_video(
        ...
        for frame_idx in tqdm(processing_order, desc="propagate in video"):
            batch_size = self._get_obj_num(inference_state) # MOD: Get all objects, including new ones
            ...
            for obj_idx in range(batch_size):
                ...
                if window_size != 0:
                    ...
                    oldest_allowed_idx = frame_idx - window_size
            ...
            if len(pred_masks_per_obj) > 1:
                ...
                if pred_masks_per_obj:
                    ...
                    all_pred_masks = torch.empty(1, 1, 256, 256)
                    empty_mask = True
            _, video_res_masks = self._get_orig_video_res_output(
                inference_state, all_pred_masks
            )
            if empty_mask:
                inference_state["masks"] = None
            else:
                inference_state["masks"] = video_res_masks
