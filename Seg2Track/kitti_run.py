...
subset = params["subset"]  # can be "training" or "testing"
base_path = params["base_path"]
detections_path = params["detections_path"]
num_sequences = {'training': 21, 'testing': 29}  # number of sequences to process

for i in range(0, num_sequences[subset]):
    print(f"Sequence {i:04d}")

    frames_dir = f"{base_path}/{subset}_jpg/image_02/{i:04d}" # Input frames directory
    output_folder = f"{base_path}/output" # Output base directory
    detections_file = f"{detections_path}/{i:04d}.txt"        
    ...
    Seg2Track(detections_file, predictor, inference_state, frames_dir, params, output_folder, sequence=f"{i:04d}")
